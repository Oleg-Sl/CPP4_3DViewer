...
Facade::Facade(BaseFileReader& file_reader_, SceneDrawerBase& scene_drawer_, MySettings& settings_) : file_reader(file_reader_), scene_drawer(scene_drawer_), settings(settings_) {
}

SceneParameters Facade::GetSettings() {
...
void Facade::LoadScene(const QString& path) {
    scene = file_reader.ReadScene(path.toStdString(), params);
    SetScene();
}
